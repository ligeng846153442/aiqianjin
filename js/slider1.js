...
Slider.prototype.createUI= function(){
	...
	for(let i=0;i<this.imgs.length;i++){
		let $img = $('<a href="#"><img/></a>');
//		$img.attr("src",this.imgs[i]);
		$img.css({
			"position":"absolute",
			"top":"0px",
			width: this.width+"px",	
			height:this.height-"113"+"px",	
...
Slider.prototype.showImg = function(inOrd,outOrd){
	...
	//1)、滑入滑出前的准备工作
	this.$imgs.eq(inOrd).css({"left":-1*this.direction * this.width+"px"});
	...
	this.$imgs.eq(outOrd).animate({
		left:this.width*this.direction
	},300);
}
...
Slider.prototype.goImg=function(transOrd){//1
	...
	if(this.currOrd>outOrd){
		...
		this.direction = 1;
		setTimeout(function(){this.direction = -1},300);
	}
